...
class FileSocket:
    ...
    def __get_ip(self, raw_ip):
        """
        Use self.__reverse() to return a readable IP address.
    ...
    def __set_inode_number(self, link_name):
        """
        'Get' and 'set' the inode number of the provided socket in the /proc/PID/fd directory.
        :return: socket inode number.
        """
        self.__ino = link_name[8:len(link_name) - 1]

    def __parse(self, data, proto="TCP"):
        """
        ...
        print("\n[*] Process name: {0}\tPID: {1} \tUID: {2}\tInode: {3}".format(self.__get_proc_name(),
                                                                                self.pid,
                                                                                uid,
                                                                                self.__ino))
    ...
    def get_ip_and_port(self, address):
        ...
        return "{}:{}".format(self.__get_ip(raw_ip), int(raw_port, base=16))
    ...
    def get_tcp_type(self):
        ...
        """
        retData = None
        socket_info = self.__get_socket_info("/proc/net/tcp")

        if socket_info:
            retData = 4, socket_info
        else:
            socket_info = self.__get_socket_info("/proc/net/tcp6")
    ...
    def get_udp_type(self):
        ...
        """
        retData = None
        socket_info = self.__get_socket_info("/proc/net/udp")

        if socket_info:
            retData = 4, socket_info
        else:
            socket_info = self.__get_socket_info("/proc/net/udp6")

            if socket_info:
                retData = 6, socket_info
            else:
                socket_info = self.__get_socket_info("/proc/net/udplite")

                if socket_info:
                    retData = "4lite", socket_info
                else:
                    socket_info = self.__get_socket_info("/proc/net/udplite6")
    ...
    def parse_udp(self):
        ...
        if data is None:
            ...
            self.__parse(data[1], "UDP")
        elif data[0] == 6:
            self.__parse(data[1], "UDP6")
        elif data[0] == "4lite":
            self.__parse(data[1], "UDP-Lite")
        else:
            self.__parse(data[1], "UDP6-Lite")
        ...
        for file in os.listdir(path):
            ...
            try:
                ...
                if "socket" in link_name:
                    self.__set_inode_number(link_name)
                    self.parser()
